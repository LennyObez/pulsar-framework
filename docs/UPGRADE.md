# Upgrade Guide: 0.x to 1.0.0-rc.1

This guide covers the migration path from Pulsar 0.x (pre-alpha/alpha) to 1.0.0-rc.1. Version 1.0.0-rc.1 marks the first release candidate with a formal public API surface and semver guarantees.

## What 1.0.0-rc.1 Means

This is a release candidate. The public API is frozen and covered by semantic versioning guarantees. No breaking changes will be introduced between rc.1 and the final 1.0.0 release unless a critical defect is discovered.
...
If your code depends on any class not marked `#[Api]`, you are depending on an internal implementation detail that may change in any minor release. Review the [Public API Reference](PUBLIC_API.md) for the complete list.
...
No namespaces were renamed in 1.0.0-rc.1. All classes remain under the `Pulsar\` root namespace. If you are upgrading from 0.2.x or earlier, the following namespaces were added in the 0.3.0-0.9.0 series:

- `Pulsar\Api` -- API stability attributes (added in 1.0.0-rc.1).
...
```bash
composer require pulsar/framework:^1.0.0-rc.1
```
...
No formal deprecations exist in 1.0.0-rc.1. The `#[Api]` / `#[Internal]` boundary replaces the informal "probably stable" / "probably internal" convention used in 0.x.
...
- Review the [Public API Reference](PUBLIC_API.md) for semver-stable types.
