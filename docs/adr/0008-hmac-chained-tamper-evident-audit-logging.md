...
- **Restart continuity, fail-closed on corruption.** If the sink implements `AuditChainStateAware`, the `AuditLogger` reads `chainState()` on construction and:
