...
For the complete public API reference, see [`docs/public-api.md`](public-api.md).
...
First-party observability and debugging extension. See [`docs/studio.md`](studio.md) for full documentation.
...
For full details on Fiber usage, extension constraints, and framework guarantees, see [`docs/async-model.md`](async-model.md).
...
Configuration via `config/queue.php`. See [`docs/cli-reference.md`](cli-reference.md) for queue commands.
...
- `ManifestSigner` - keyed BLAKE2b signing and verification using a derived subkey (subKeyId=6, context=`integ_sg`)
