...
Tenant IDs must match the pattern `[a-zA-Z0-9_-]+` (alphanumeric characters, hyphens, and underscores only). Empty IDs are rejected. The constructor throws `InvalidArgumentException` if the ID contains invalid characters. This constraint prevents SQL injection and path traversal when tenant IDs are interpolated into table prefixes, connection names, or file paths.
...
`TenantContext` implements `ResettableInterface`, which ensures it is automatically cleared between requests in the persistent runtime. The `RequestSandbox` calls `reset()` on all registered resettable services at the end of each request lifecycle. This prevents tenant state from leaking between consecutive requests handled by the same long-lived worker process.
