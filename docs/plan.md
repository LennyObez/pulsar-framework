...
### 2.49 Funding and sponsorship model: GitHub Sponsors + Open Collective baseline; optional dual-licence commercial tier post-GA

The funding model is a multi-year solo-friendly hybrid that does not gate framework progress on funding milestones. The minimum baseline at every milestone is **GitHub Sponsors** (individual recurring tier and corporate one-shot tier) plus an **Open Collective** fiscal host enabling tax-deductible corporate contributions in jurisdictions that require an invoiceable entity. Both channels open at Phase 0 namespace publication (`v0.0.1-alpha.0`) and remain the only solicitation channels through GA. No paid features, no pay-to-prioritise tier, no advertising, no donor-named modules.

A second tier becomes available **post-GA only**: an optional dual-licence path under which selected organisations may obtain a commercial-use licence (Apache-2.0 remains the only OSS licence; the commercial path is a parallel grant covering the trademark, indemnification, and prioritised support, not a different code licence). Eligibility scoped to regulated organisations whose procurement requires a counterparty contract. Pricing tiered by organisation size and modelled on Tidelift's commercial-tier conventions for high-assurance OSS. The dual-licence path is **not** required to consume Pulsar; it is a procurement-friendly contractual wrapper that some regulated buyers need.

Rationale: a multi-year solo project requires sustainable but realistic funding without compromising the OSS surface. GitHub Sponsors plus Open Collective covers the realistic year-one to year-three solo-maintainer income floor; the post-GA dual-licence path opens revenue surface from the regulated-domain audience (banking, healthcare, legal, government) that prefers contractual relationships, without restricting the OSS code path. Alternatives considered: VC funding (rejected: changes the optimisation function from "ship when correct" to "ship when ARR target hit", incompatible with Decision 2.33); open-core commercial split (rejected: bifurcates the OSS surface and creates incentive to under-build OSS features); donor-named modules (rejected: corrupts module-naming conventions in Section XVII); ad-supported (rejected: incompatible with regulated-domain audience expectations).

### 2.50 Trademark and patent posture: EUIPO trademark, defensive Apache-2.0 patent grant, no offensive patents

The trademark and patent posture is asymmetric and explicit:

**Trademark.** "Pulsar Framework" registered with the European Union Intellectual Property Office (EUIPO) under the relevant trademark classes for software and software-as-a-service (Class 9, Class 42). Registration timeline: filing during Phase 0 (concurrent with the namespace-reservation alpha publish at `v0.0.1-alpha.0`); full registration target before `v0.5.0` core release. The trademark protects the name and any associated word-and-design marks across the EU single market; subsequent international extensions through Madrid Protocol (WIPO) at `v1.0.0` GA targeting at minimum United Kingdom, United States, Switzerland, Canada, Australia, Singapore, Japan. The trademark policy in `docs/trademark-policy.md` governs permitted community uses (forks may use the name to identify they are forks; downstream applications may not use the mark in their own product name without a community licence; commercial entities require explicit permission for production use of the mark in marketing materials).

**Patent posture.** Pulsar Framework is licensed under Apache-2.0 (Decision 2.21), which incorporates an explicit patent grant: every contributor automatically grants a perpetual, worldwide, non-exclusive, no-charge, royalty-free, irrevocable patent licence to make, use, sell, offer to sell, import, and otherwise transfer the work covering any patent claims they own that read on the contribution. The defensive termination clause activates if a licensee initiates patent litigation alleging that the work or a contribution infringes their patents.

**No offensive patents.** Pulsar Framework will not file software patents on its own technology (formally verified router trie, audit chain, capability table, etc.) and will not enforce any patent claims that arise as a side effect of contributors' grants. The maintainer commits in writing (this decision artefact and the `docs/patent-non-aggression.md` published at GA) to a non-aggression pledge: no offensive patent assertion against any user of Pulsar Framework, and no participation in patent troll arrangements. The defensive Apache-2.0 termination clause is the sole patent enforcement mechanism.
...
**HACL\* FFI binding crate (+1 net):** Per Decision 2.53 the HACL\* formally-verified cryptographic library is wrapped in a `*-sys`-style binding crate `pulsar-crypto-hacl-bindings` (per Rust ecosystem convention for C-binding crates). The crate vendors the HACL\* C distribution under `hacl-c/`, compiles it via `build.rs` + `cc`, generates Rust FFI declarations via `bindgen`. Per Decision 2.60 the binding crate is scoped to the **12 classical primitives only** (AEAD, X25519, P-256, Ed25519, SHA-2/3, BLAKE2, HKDF, HMAC); the 4 NIST PQC primitives (ML-KEM-768/1024, ML-DSA-65/87) are sourced from `libcrux-ml-kem` + `libcrux-ml-dsa` Rust-native workspace dependencies — they do not require an FFI binding crate. Safe Rust wrappers + Creusot contracts + capability gating live in `pulsar-kernel` (which depends on `pulsar-crypto-hacl-bindings` and on the libcrux PQC workspace deps directly).
...
### 2.53 Kernel cryptographic primitives: HACL\* via FFI for the classical surface (formally verified, constant-time proved)

The **classical** cryptographic primitives in `pulsar-kernel` are sourced from **HACL\*** — the formally verified cryptographic library written in F\* and compiled to optimised constant-time C code. HACL\* is exposed to Rust through a `*-sys`-style FFI binding crate `pulsar-crypto-hacl-bindings` (new in v2.3, Sprint 1.1). The Rust-side `pulsar-kernel::crypto` module re-exports the primitive API, layered with sealed-trait safety wrappers and Creusot contracts on the lifecycle invariants. The PQC surface (ML-KEM, ML-DSA) and the password-hashing surface (Argon2id) live in companion crates (per Decision 2.60 multi-source verification stack).

**Primitives sourced from HACL\* (12 — classical surface):** AES-128-GCM, AES-256-GCM, ChaCha20-Poly1305, Curve25519 (X25519), Ed25519, P-256, SHA-2 (256/384/512), SHA-3 (256/384/512), HKDF, HMAC, BLAKE2b, BLAKE2s. The primitives ML-KEM-768/1024 and ML-DSA-65/87 are NOT in HACL\* upstream as of 2026Q2 — they are sourced from libcrux per Decision 2.60.

**Primitives kept on companion verified / audited crates:** Argon2id (KDF — RustCrypto `argon2`, audited canonical Rust implementation), `subtle::ConstantTimeEq` (constant-time byte comparison primitives — `subtle` is already pulled by HACL\* internally), legacy-compat AEADs needed for backwards compatibility with PHP-era stored material (AES-256-CBC + HMAC-SHA256 composite, deprecated at 1.0 but present at v0.9.x for migration tooling), and the four PQC primitives sourced from libcrux per Decision 2.60.
...
Rationale: for a framework targeting banking + healthcare + government + legal where cryptographic-defect blast-radius is denominated in regulatory fines and licensure loss, the gap between "audited" and "formally verified" is the gap regulated procurement evaluates. HACL\* closes that gap at the classical primitives layer for a modest engineering cost (FFI binding maintenance + an additional C compiler in the build pipeline). Combined with Decision 2.20 (Creusot + TLA+ for the Rust-side kernel composition) and Decision 2.60 (libcrux for PQC primitives — same Cryspen / hax / F\* verification provenance), Pulsar's cryptographic surface reaches the verification tier of high-assurance research kernels for all sixteen targeted primitives.

Alternatives considered: keep `ring` + `subtle` only (v2.2 baseline) — rejected: leaves the formal-verification gap; HACL\*-only for all sixteen primitives — rejected: HACL\* upstream does not ship NIST ML-KEM + ML-DSA as of 2026Q2 (Frodo + K256 are present but not the FIPS 203/204 winners); adopt liboqs for PQC primitives — rejected: liboqs is C without formal verification; libcrux (Rust-native, hax + F\* verification, same Cryspen team as HACL\*) provides the four PQC primitives with the same verification posture and zero FFI burden, settled per Decision 2.60; write our own crypto in pure Rust with Creusot proofs — rejected: years of effort, and even with proofs would lack production deployment evidence; use BoringSSL or libsodium — rejected: BoringSSL is C without formal proofs, libsodium is C audited but not formally verified.
...
**Key establishment (KEM):** every protocol that uses key encapsulation runs **X25519 + ML-KEM-768** in parallel and combines the resulting shared secrets via HKDF (SHA-256). This is the construction used by Cloudflare PQ-Hybrid, Google Chrome PQ-TLS, Apple iMessage PQ3, and IETF draft-ietf-tls-hybrid-design. ML-KEM-768 corresponds to NIST FIPS 203 (final August 2024). X25519 is sourced from HACL\* via FFI (per Decision 2.53). ML-KEM-768 is sourced from `libcrux-ml-kem` (Rust-native, hax + F\* verification, FIPS 203 conformance — per Decision 2.60 multi-source verification stack).

**Signatures:** every signature scheme that anchors a long-term commitment runs **Ed25519 + ML-DSA-65** in parallel and verifies require BOTH to pass. ML-DSA-65 corresponds to NIST FIPS 204. Ed25519 is sourced from HACL\* via FFI (per Decision 2.53). ML-DSA-65 is sourced from `libcrux-ml-dsa` (Rust-native, hax + F\* verification, FIPS 204 conformance — per Decision 2.60). The double-signature posture is more conservative than single-PQC-only because it preserves classical-only verifiability for legacy clients while gaining PQC protection.
...
### 2.60 Multi-source formally-verified crypto stack: HACL\* (classical) + libcrux (PQC) + RustCrypto (Argon2id)

The cryptographic surface in `pulsar-kernel::crypto` is sourced from a **multi-source formally-verified crypto stack** rather than a single library: **HACL\*** for the twelve classical primitives (FFI-bound through `pulsar-crypto-hacl-bindings`), **libcrux** for the four NIST PQC primitives (Rust-native crates `libcrux-ml-kem` + `libcrux-ml-dsa`), and **RustCrypto `argon2`** for the Argon2id password-hashing KDF (audited canonical Rust implementation outside the formal-verification boundary). All sixteen formally-verified primitives share **the same verification provenance** — Cryspen's hax + F\* tooling — even though distribution is split between C extraction (HACL\*) and Rust extraction (libcrux). Argon2id is the only audited-but-not-verified primitive in the stack and is explicitly scoped (not used for key material requiring formal verification).

**Why multi-source is the correct posture:** HACL\* upstream as of 2026Q2 ships verified C for the classical primitives + Frodo (a non-NIST PQC scheme) + K256 ECDSA, but **does not** ship NIST ML-KEM-768/1024 or ML-DSA-65/87. The same Cryspen team that develops HACL\* maintains libcrux as a parallel verified-Rust ecosystem where the NIST PQC primitives are extracted: `libcrux-ml-kem` exposes ML-KEM-512/768/1024 with FIPS 203 conformance and machine-checked proofs of "field arithmetic, NTT polynomial arithmetic, serialization, and the generic code for high-level algorithms" via hax + F\*; `libcrux-ml-dsa` exposes ML-DSA-44/65/87 with the same verification posture for FIPS 204. Single-sourcing through HACL\* would either (a) require waiting for HACL\* upstream to add ML-KEM/ML-DSA — no published timeline — or (b) Pulsar maintaining its own F\* extraction of the NIST primitives — years of effort. Multi-sourcing through libcrux closes the gap immediately with no verification compromise (same toolchain, same team, same provenance).

**Sourcing per primitive family:**

| Family | Primitives | Source | Verification | Distribution |
|--------|-----------|--------|--------------|--------------|
| AEAD | AES-128/256-GCM, ChaCha20-Poly1305 | HACL\* | F\* proofs (correctness + memory + constant-time) | C via FFI (`pulsar-crypto-hacl-bindings`) |
| Key agreement (classical) | Curve25519 (X25519), P-256 | HACL\* | F\* proofs | C via FFI |
| Signature (classical) | Ed25519 | HACL\* | F\* proofs | C via FFI |
| Hash | SHA-2-256/384/512, SHA-3-256/384/512, BLAKE2b, BLAKE2s | HACL\* | F\* proofs | C via FFI |
| KDF / MAC | HKDF (SHA-2), HMAC | HACL\* | F\* proofs | C via FFI |
| KEM (PQC) | ML-KEM-768, ML-KEM-1024 | libcrux | hax + F\* proofs (FIPS 203 conformance) | Rust crate `libcrux-ml-kem` |
| Signature (PQC) | ML-DSA-65, ML-DSA-87 | libcrux | hax + F\* proofs (FIPS 204 conformance) | Rust crate `libcrux-ml-dsa` |
| Password KDF | Argon2id | RustCrypto `argon2` | Audited canonical Rust impl (not formally verified) | Rust crate (audit-only scope) |

**Why provenance unification matters:** HACL\* and libcrux are both Cryspen-maintained projects. They share the F\* / hax extraction toolchain, the same proof methodology (functional correctness + memory safety + secret independence), and the same upstream maintainers. Cross-project verification audits (e.g., third-party CCA evaluations) treat HACL\* + libcrux as a single verification surface for procurement evaluation purposes. The sourcing split is a distribution detail (C vs Rust extraction), not a verification compromise.

**Build implications:** `pulsar-kernel` depends on `pulsar-crypto-hacl-bindings` (C compiler + `cc` + `bindgen` build pipeline for HACL\*), `libcrux-ml-kem` + `libcrux-ml-dsa` (pure Rust dependencies — no FFI burden, no C toolchain required for PQC), and `argon2` (pure Rust). The PQC dependencies pin specific libcrux release versions in workspace `Cargo.toml` for SLSA L4 reproducibility per Decision 2.57. Workspace `cargo deny` enforces both Cryspen / hax-verified releases and the audit posture for `argon2`.

**SLSA L4 reproducibility model — hybrid distribution.** The two formally-verified sources reach SLSA Level 4 reproducibility through different but complementary mechanisms: (a) **HACL\* C distribution** is *vendored* under `crates/pulsar-crypto-hacl-bindings/hacl-c/` with a pinned upstream commit SHA + tarball SHA-256 captured in repository for byte-identical reconstruction (~10-15 MB selective vendoring per Decision 2.57); (b) **libcrux Rust crates** are *fetched from crates.io* as normal cargo dependencies with the resolved package SHA-256 captured in `Cargo.lock` for byte-identical reconstruction. Both reproducibility models are auditable independently — `Cargo.lock` covers libcrux determinism; the vendored `hacl-c/` directory + its checksum manifest covers HACL\* determinism. Cryspen publishes libcrux to crates.io via a deterministic CI pipeline; the chain of custody from F\* / hax extraction to crates.io is independently verifiable through Cryspen's release attestations. The hybrid model is not a regression of SLSA L4 — both sources reach L4, via the appropriate pinning mechanism for their distribution channel.

Rationale: this decision formalises the sourcing split made visible during Sprint 0.9-bis closure when the v2.3 reconciliation audit identified that the HACL\*-only assumption in Decision 2.53 (drafted before the actual upstream HACL\* primitive set was verified) was inaccurate. Rather than weaken the verification posture (single-source HACL\* without NIST PQC) or wait for HACL\* upstream to add ML-KEM/ML-DSA (no timeline), Decision 2.60 anchors the multi-source posture with a unified provenance argument. The resulting stack covers all sixteen formally-verified primitives with the strongest available verification posture per primitive family.

Alternatives considered: HACL\*-only sourcing — rejected: HACL\* upstream does not ship NIST ML-KEM/ML-DSA as of 2026Q2; defer PQC to Sprint 1.5 or later — rejected: contradicts Decision 2.58 hybrid PQC from Sprint 1.1 + the harvest-now-decrypt-later motivation; libcrux for everything (drop HACL\*) — rejected: HACL\*'s production deployment evidence (Mozilla NSS + Linux WireGuard + Tezos + Azure VPN + Zcash) is unmatched in the verified-Rust ecosystem, and libcrux is younger; RustCrypto for everything (drop both HACL\* and libcrux) — rejected: RustCrypto is audited but not formally verified, strictly weaker on the primitives where verification matters; mix HACL\* + liboqs for PQC — rejected: liboqs is C without formal verification, strictly weaker than libcrux on the same primitive set.
...
### 4.2.5 pulsar-crypto-hacl-bindings (HACL\* FFI binding crate, classical surface — v2.3 lock-in per Decision 2.53 + Decision 2.60 + ADR-0009)

**Purpose.** `*-sys`-style FFI binding crate that vendors the HACL\* C distribution under `hacl-c/` and exposes Rust FFI declarations for the **classical** formally-verified cryptographic primitives `pulsar-kernel::crypto` re-exports under safety wrappers + Creusot contracts + capability gating. The four PQC primitives (ML-KEM-768/1024, ML-DSA-65/87) are sourced from the libcrux Rust-native crates per Decision 2.60 — they do **not** flow through this FFI binding crate.

**Primitives wrapped (12 — classical surface).** AEAD: AES-128-GCM, AES-256-GCM, ChaCha20-Poly1305. Key agreement (classical): Curve25519 (X25519), P-256 (NIST). Signature (classical): Ed25519. Hashes: SHA-2-256/384/512, SHA-3-256/384/512, BLAKE2b, BLAKE2s. KDFs: HKDF (over SHA-2 family), HMAC.

**Primitives NOT wrapped here (sourced elsewhere).** ML-KEM-768, ML-KEM-1024 → `libcrux-ml-kem` Rust crate (per Decision 2.60). ML-DSA-65, ML-DSA-87 → `libcrux-ml-dsa` Rust crate (per Decision 2.60). Argon2id → `argon2` RustCrypto crate (per Decision 2.60 — audited, not formally verified).

**Build.** `build.rs` compiles `hacl-c/` via the `cc` crate + generates Rust FFI declarations via `bindgen` for the classical primitive surface only. At Phase 0 the `hacl-c/` directory is a placeholder; Sprint 1.1 (kernel crypto) lands the actual extracted C sources from the upstream HACL\* repository, scoped to the twelve classical primitives (vendored to ensure SLSA Level 4 reproducibility per Decision 2.57).
...
| Formally-verified cryptographic primitives (multi-source stack per Decision 2.60) | 16 total: 12 classical from HACL\* via FFI (AES-128/256-GCM, ChaCha20-Poly1305, Curve25519, Ed25519, P-256, SHA-2/3 256/384/512, HKDF, HMAC, BLAKE2) + 4 PQC from libcrux Rust-native (ML-KEM-768/1024, ML-DSA-65/87) — same Cryspen / hax / F\* verification provenance | `pulsar-crypto-hacl-bindings` (HACL\* FFI) + `libcrux-ml-kem` + `libcrux-ml-dsa` (Rust-native) |
...
| PQC posture                                | Hybrid X25519 + ML-KEM-768 for KEM, Ed25519 + ML-DSA-65 for signatures, from Sprint 1.1 (per Decision 2.58 v2.3 lock-in — no classical-only window). Classical primitives via HACL\* FFI; PQC primitives via libcrux Rust-native (per Decision 2.60). | `pulsar-kernel::crypto` (HACL\* + libcrux composition) |
...
### 14.1 Post-quantum cryptography (lifted to Sprint 1.1 per Decision 2.58; sourcing per Decision 2.60)

**Scope.** Introduces hybrid key exchange combining classical X25519 with ML-KEM-768 at the TLS 1.3 layer + protocol surface, and hybrid signatures combining Ed25519 with ML-DSA-65 at the code-signing and token-signing layers. Adopts the NIST Post-Quantum Cryptography standardised primitives published as FIPS 203 (ML-KEM), FIPS 204 (ML-DSA), and FIPS 205 (SLH-DSA — opt-in for mathematical-assumption diversity). The `pulsar-kernel::crypto` module exposes hybrid variants of each primitive; deployments opt in via configuration during a transition window; the classical-only variants are deprecated at 2.0.
...
**Dependency note.** Depends on `rustls` PQ-hybrid support reaching stable; on `libcrux-ml-kem` (FIPS 203, hax + F\* verified, per Decision 2.60) and `libcrux-ml-dsa` (FIPS 204, hax + F\* verified, per Decision 2.60) reaching the Pulsar-pinned release versions; on the HACL\* C distribution vendored under `pulsar-crypto-hacl-bindings/hacl-c/` covering X25519 + Ed25519 (per Decision 2.53). The pre-2.3 `pqcrypto-*` ecosystem dependency is no longer required (libcrux supersedes it).
