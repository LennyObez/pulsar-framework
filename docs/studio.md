...
return [
    ...
    'store' => 'sqlite', // 'sqlite', 'database', or 'buffered'
...
| `STUDIO_STORE_BACKEND`      | `store`                       | string |
...
Studio supports three storage backends, configured via the `store` key in `config/studio.php` or the `STUDIO_STORE_BACKEND` environment variable.
...
Uses a dedicated SQLite file (configured via `storage_path`) with WAL mode. Recommended for local development and low-traffic staging environments. Zero external dependencies. Chain linearization uses `BEGIN IMMEDIATE` with exponential backoff retry on `SQLITE_BUSY`.
...
Uses the application's primary database connection (`ConnectionInterface`) for event storage. Recommended for production deployments where events should live alongside application data in PostgreSQL or MySQL. Chain linearization uses `SELECT ... FOR UPDATE` on PostgreSQL/MySQL for row-level locking. Falls back to SQLite's implicit serialization when the application database is SQLite.

The database backend requires the `studio_events` and `studio_chain` tables to exist. Run the Studio migration or create them manually with the same schema as the SQLite store.
...
Wraps the database backend with an in-memory buffer (default: 100 events). Events accumulate in memory and flush to the database in batch when the buffer is full, on read operations, or at process shutdown. Recommended for high-traffic production environments where per-event write latency is unacceptable.

The buffered backend eliminates write contention from the request hot path at the cost of potential event loss on unclean process termination (e.g., `kill -9`). Events are best-effort in buffered mode.
