...
final readonly class DbCmsUserRepository implements CmsUserRepositoryInterface
{
    ...
    public function listUsers(
        ...
        $total = (int) ($countResult->first()['total'] ?? 0);
        ...
        $items = array_map($this->hydrate(...), $result->all());
     ...
     */
    private function hydrate(array $row): CmsUser
    {
        $rolesRaw = $row['roles'] ?? '[]';
        /** @var list<string> $roles */
        $roles = is_string($rolesRaw) ? (array) json_decode($rolesRaw, true) : [];

        $twoFactorRaw = is_string($row['two_factor_status'] ?? null)
            ? $row['two_factor_status']
            : 'disabled';
        ...
        return new CmsUser(
            ...
            contentCount: (int) ($row['content_count'] ?? 0),
            commentCount: (int) ($row['comment_count'] ?? 0),
            lastActiveAt: is_string($row['last_active_at'] ?? null)
