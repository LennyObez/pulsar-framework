...
final readonly class MysqlSearchService implements SearchServiceInterface
{
    private const string SQL_SEARCH = <<<'SQL'
        SELECT c.*,
            MATCH(ct.title, ct.body) AGAINST(:query IN BOOLEAN MODE) AS ft_rank
        FROM cms_content_translations ct
        INNER JOIN cms_contents c ON c.id = ct.content_id
        WHERE MATCH(ct.title, ct.body) AGAINST(:query IN BOOLEAN MODE)
    ...
    public function search(
        ...
        $searchSql .= ' ORDER BY ft_rank DESC LIMIT :fetch_limit';
        ...
        // Hydrate content with FULLTEXT rank
        $ranked = [];
        $contentIds = [];

        foreach ($result->rows as $row) {
            $content = self::hydrateContent($row);
            $ftRank = $row->getFloat('ft_rank');
            $contentIds[] = $content->id;
            $ranked[] = ['content' => $content, 'ft_rank' => $ftRank];
        }
