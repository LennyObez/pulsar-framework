...
return new class implements MigrationInterface {
    public function up(ConnectionInterface $connection): void
    {
        ...
        $connection->execute(<<<'SQL'
            CREATE INDEX idx_content_status_tenant ON cms_contents (status, tenant_id)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_content_type_status_tenant ON cms_contents (content_type, status, tenant_id)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_content_parent_sort ON cms_contents (parent_id, sort_order)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_content_scheduled_publish ON cms_contents (scheduled_publish_at)
        ...
        $connection->execute(<<<'SQL'
            CREATE UNIQUE INDEX uq_translation_locale_tenant_path
                ON cms_content_translations (locale, tenant_key, path)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE UNIQUE INDEX uq_translation_content_locale
                ON cms_content_translations (content_id, locale)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_translation_search_vector
