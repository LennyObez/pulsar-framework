...
return new class implements MigrationInterface {
    public function up(ConnectionInterface $connection): void
    {
        ...
        $connection->execute(<<<'SQL'
            CREATE INDEX idx_media_file_hash ON cms_media_assets (file_hash)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_media_tenant_created ON cms_media_assets (tenant_id, created_at DESC)
            SQL);
        ...
        $connection->execute(<<<'SQL'
            CREATE TABLE cms_media_derivatives (
                id VARCHAR(36) NOT NULL,
                media_asset_id VARCHAR(36) NOT NULL,
