...
return new class implements MigrationInterface {
    public function up(ConnectionInterface $connection): void
    {
        $connection->execute(<<<'SQL'
            CREATE TABLE cms_content_type_fields (
        ...
        $connection->execute(<<<'SQL'
            CREATE UNIQUE INDEX uq_content_type_field_key
        ...
        $connection->execute(<<<'SQL'
            CREATE UNIQUE INDEX uq_field_value_content_field_locale
                ON cms_content_field_values (content_id, field_id, COALESCE(locale, ''))
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_field_value_string ON cms_content_field_values (field_id, value_string)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_field_value_int ON cms_content_field_values (field_id, value_int)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_field_value_datetime ON cms_content_field_values (field_id, value_datetime)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_field_value_bool ON cms_content_field_values (field_id, value_bool)
            SQL);
    }
