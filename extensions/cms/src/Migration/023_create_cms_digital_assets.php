...
return new class implements MigrationInterface {
    public function up(ConnectionInterface $connection): void
    {
        $connection->execute(<<<'SQL'
            CREATE TABLE cms_digital_assets (
        ...
        $connection->execute(<<<'SQL'
            CREATE INDEX idx_digital_asset_product ON cms_digital_assets (product_id)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE TABLE cms_digital_downloads (
                id VARCHAR(36) NOT NULL,
        ...
        $connection->execute(<<<'SQL'
            CREATE UNIQUE INDEX uq_download_token
                ON cms_digital_downloads (download_token)
            SQL);

        $connection->execute(<<<'SQL'
            CREATE INDEX idx_download_order_item ON cms_digital_downloads (order_item_id)
            SQL);
    }
