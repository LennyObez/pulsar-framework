        ...
        foreach (self::TABLES as $spec) {
            ...
            match ($driver) {
                Driver::MySQL => $connection->execute(
                    "CREATE INDEX {$spec['index']} ON {$spec['table']} (deleted_at)",
                ),
                Driver::SQLite, Driver::PostgreSQL => $connection->execute(
                    "CREATE INDEX IF NOT EXISTS {$spec['index']} ON {$spec['table']} (deleted_at)",
                ),
            };
        }
    }
        ...
        foreach (self::TABLES as $spec) {
            match ($driver) {
                Driver::SQLite => $connection->execute(
                    "DROP INDEX IF EXISTS {$spec['index']}",
                ),
                Driver::MySQL => $connection->execute(
                    "ALTER TABLE {$spec['table']} DROP INDEX {$spec['index']}",
                ),
