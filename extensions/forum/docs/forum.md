...
| `ipHash`              | `string`             | Hashed IP (anti-abuse, no PII)       |
| `userAgentHash`       | `string`             | Hashed user agent (anti-abuse)       |
| `editedAt`            | `?DateTimeImmutable` | When the post was last edited        |
...
Vote records for threads and posts. Each user can cast one vote per target. Unique per (tenant, user, target) enforced at the repository/DB level.
...
| `threadId` / `postId` | `string`            | FK to thread or post     |
| `value`               | `VoteDirection`     | `Up` (+1) or `Down` (-1) |
...
Thread follow records for notification delivery. Unique per (tenant, user, thread) enforced at the repository/DB level.
...
| `userId`    | `string`            | FK to auth user     |
| `threadId`  | `string`            | FK to Thread        |
| `createdAt` | `DateTimeImmutable` | Subscription time   |
...
### Link density and similarity detection
