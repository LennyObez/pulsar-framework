@extends('errors.layout')

@section('theme', 'light')
@section('title')@t('errors.401.title')@endsection

@section('content')
    <h1 class="error-page__heading" id="error-title" data-t="errors.401.heading">
        @t('errors.401.heading')
    </h1>
    <p class="error-page__description" data-t="errors.401.description">
        @t('errors.401.description')
    </p>
@endsection

@section('actions')
    <a class="error-page__btn error-page__btn--primary" href="/login" data-t="errors.sign_in">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"/><polyline points="10 17 15 12 10 7"/><line x1="15" y1="12" x2="3" y2="12"/></svg>
        @t('errors.sign_in')
    </a>
    <a class="error-page__btn error-page__btn--secondary" href="/" data-t="errors.go_home">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
        @t('errors.go_home')
    </a>
    <a class="error-page__btn error-page__btn--ghost" href="javascript:history.back()" data-t="errors.go_back">
        @t('errors.go_back')
    </a>
@endsection
