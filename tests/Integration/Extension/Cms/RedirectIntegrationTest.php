...
final class InMemoryRedirectRepository implements RedirectRepositoryInterface
{
    ...
    public function findByPath(string $path, ?string $locale = null, ?string $tenantId = null): ?Redirect
    {
        foreach ($this->redirects as $redirect) {
            if ($redirect->fromPath === $path && $redirect->deletedAt === null) {
                return $redirect;
            }
        }
    ...
    public function findAll(int $page = 1, int $perPage = 50, ?string $tenantId = null): array
    {
        $items = array_values(array_filter(
            $this->redirects,
            static fn(Redirect $r): bool => $r->deletedAt === null,
        ));
    ...
    {
        if (isset($this->redirects[$redirectId])) {
            $r = $this->redirects[$redirectId];
            $this->redirects[$redirectId] = new Redirect(
                id: $r->id,
                tenantId: $r->tenantId,
                fromPath: $r->fromPath,
                toPath: $r->toPath,
    ...
    public function purgeDeleted(DateTimeImmutable $olderThan): int
    {
        ...
        foreach ($this->redirects as $id => $redirect) {
            if ($redirect->deletedAt !== null && $redirect->deletedAt < $olderThan) {
