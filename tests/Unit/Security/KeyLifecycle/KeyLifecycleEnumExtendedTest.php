...
final class KeyLifecycleEnumExtendedTest extends TestCase
{
    // ── CertificateType ────────────────────────────────────────────
    ...
    #[Test]
    public function certificateTypeFromRoundTrips(): void
    {
        foreach (CertificateType::cases() as $type) {
            self::assertSame($type, CertificateType::from($type->value));
        }
    }
    ...
    #[Test]
    public function warningLevelHasFourCases(): void
    {
    ...
    public function warningLevelBackedValues(CertificateWarningLevel $level, string $expected): void
    {
    ...
    #[Test]
    public function warningLevelFromRoundTrips(): void
    {
        foreach (CertificateWarningLevel::cases() as $level) {
            self::assertSame($level, CertificateWarningLevel::from($level->value));
        }
    }

    #[Test]
    public function warningLevelTryFromReturnsNullForInvalid(): void
    {
        self::assertNull(CertificateWarningLevel::tryFrom('panic'));
    }
    ...
    #[Test]
    public function keyTypeFromRoundTrips(): void
    {
        foreach (KeyType::cases() as $type) {
            self::assertSame($type, KeyType::from($type->value));
        }
    }

    #[Test]
    public function keyTypeTryFromReturnsNullForInvalid(): void
    {
        self::assertNull(KeyType::tryFrom('unknown_key_type'));
    }
}
